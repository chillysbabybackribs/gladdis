...
export class FileTools {
  ...
  /**
   * Content search: walk `path` recursively and return lines matching the
   * query string. Lowercase queries stay broad; queries containing uppercase
   * letters become case-sensitive for better symbol precision. `glob`
   * restricts which file names are scanned (a simple * / ? glob, no path
   * separators). Bounded by MAX_ENTRIES hits and a directory-skip list for
   * the usual heavy dirs.
   */
  async search(
    query: string,
    path = '.',
  ...
  const walk = async (dir: string): Promise<void> => {
    ...
    for (const name of names) {
      ...
      if (st.isDirectory()) {
        ...
        const scan = searchFile(full, matcher, hits, limit, context, () => truncated = true)
        pending.push(scan)
