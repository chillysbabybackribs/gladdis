...
- Use this exact loop: search → fetch_page or navigate → grep_page (primary) or read_page (secondary/orientation) → act (click_xy, type_text, press_key, execute_in_browser) → verify with grep_page or read_page.
...
- Prefer click_xy with coordinates from grep_page search results.
