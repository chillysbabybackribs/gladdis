...
2. `grep_click` or `click_xy` with `@aN` refs — primary interaction path after `read_a11y`
...
- After `read_a11y`, prefer `grep_click` or `click_xy(ref)` using the returned `@aN` refs
